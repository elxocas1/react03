...
const AppForm = (props) => {
    ...
    try {
      if(props.idActual == ""){
        if(validarForm()){                            // Validación de form
          ...
          toast("Documento se registro con éxito", { type: '', autoClose: 2000})        
        }else{
        ...
        toast("Documento actualizado con éxito", { type: '', autoClose: 2000})      }
    } catch (error) {
  ...
  const validarForm = () => {
    if(objeto.nombre === "" || /^\s+$/.test(objeto.nombre)){
      toast("Nombre", { type: 'error', autoClose: 2000})
      return false;
    }
    return true;
  };
